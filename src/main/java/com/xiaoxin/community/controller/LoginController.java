...
public class LoginController implements CommunityConstant {
    ...
    // http://localhost:8080/community/activation/101/code激活
    ...
    public void getKaptcha(HttpServletResponse response/*, HttpSession session*/) {
        ...
        cookie.setMaxAge(60); //设置验证码有效时间
        cookie.setPath(contextPath); //设置有效路径
        response.addCookie(cookie);
        //将验证码存入到redis
        String redisKey = RedisKeyUtil.getKaptchaKey(kaptchaOwner);
        redisTemplate.opsForValue().set(redisKey, text,60, TimeUnit.SECONDS); //设置验证码有效时间
    ...
    public String login(String username, String password, String code, boolean rememberme,
        ...
        if (StringUtils.isNotBlank(kaptchaOwner)){
            String redisKey = RedisKeyUtil.getKaptchaKey(kaptchaOwner);
            kaptcha = (String) redisTemplate.opsForValue().get(redisKey); //从redis中取出验证码
        }
